...
function [iter, fluxes, rad, thermal, profiles, soil, RWU, frac, WaterStressFactor,WaterPotential]             ...
    ...
    L     = 100;                       %           Monin-Obukhov length (Is fixed???)) 
    % load Constants
    ...
    while CONT                          % while energy balance does not close
        ...
        resist_in.Ta = meteo.Ta + 273.15; % In Kelivin , for calculating kinematic viscosity of air.
        resist_in.Ts = Ts; %Soil temperature
       
        [resist_out]  = resistancesHF(resist_in);
        ...
        rac   = resist_out.rac;
        rash  = resist_out.rash;
        rasu  = resist_out.rasu;
        rbc   = resist_out.rbc;
        rbsh  = resist_out.rbsh;
        rbsu  = resist_out.rbsu;

        rach  = rac + rbc; % Sum of aerodynamic and boundary layer resistance canopy (shaded)
        % racu is Sum of aerodynamic and boundary layer resistance canopy (sunlight)
        racu  = permute(repmat(rach, [1, size(rcwu, 2), size(rcwu, 1)]),[3,2,1]); %same shape as rcwu 30*36*13
        % rash  = rash+rbsh; % Sum of aerodynamic and boundary layer resistance soil (shaded)
        % rasu  = rasu+rbsu; % Sum of aerodynamic and boundary layer resistance soil (sunlit)
        rss   = soil.rss;  % Stomatal resistance of soil
        % Sum of aerodynamic and boundary layer resistance soil
        ras   = [resist_out.rasu + resist_out.rbsh;resist_out.rasu + resist_out.rbsu]; % Since in Fs, [shaded , sunlight]
        ...
        Tch         = Tch + Wc * EBerch ./ ((rhoa * cp) ./ rac + rhoa * lambdah * e_to_q .* sh ./ (rach + rcwh) + 4 * leafbio.emis * sigmaSB * (Tch + 273.15).^3);
    ...
    thermal.raws  = resist_out.raws;     % [s m-1]             aerodynamic resistance below canopy for soil
    ...
    fid = fopen('/Users/prajzwal/PhD/testcase/DE-Tha_2025-04-16-1631/v4/output_resistances.csv', 'a');  % 'a' mode to append; if file doesn't exist, it'll create one
    ...
    if ftell(fid) == 0  % Check if file is empty by checking the position of the file pointer
        fprintf(fid, '%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s\n', ...
            ...
            'rar', 'raws', 'rasu', ...
            'rbsu', 'rbsh', ...
            'deltasu', 'deltash', 'lm', 'beta', ...
            'Fsh','Tsh','Tsu','Tsave','Tcave');
    ...
    fprintf(fid, '%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f\n', ...
        ...
        resist_out.ustar,  resist_out.us,  resist_out.d, ...
        resist_out.rar, resist_out.raws,resist_out.rasu, ...
        resist_out.rbsu, resist_out.rbsh, ...
        resist_out.deltasu, resist_out.deltash, resist_out.lm,resist_out.beta, ...
        Fs(1), Ts(1), Ts(2), Tsave, Tcave);

    
    % Close the file after the operation
    fclose(fid);
    % Paths for output files
    rac_file = '/Users/prajzwal/PhD/testcase/DE-Tha_2025-04-16-1631/v4/rac.csv';
    rbc_file = '/Users/prajzwal/PhD/testcase/DE-Tha_2025-04-16-1631/v4/rbc.csv';
    
    % Ensure rac and rbc are column vectors
    rac_col = resist_out.rac(:);  % 30x1
    rbc_col = resist_out.rbc(:);  % 30x1
    
    % Append to rac.csv
    if exist(rac_file, 'file')
        % Load existing data and append new column
        existing_rac = readmatrix(rac_file);
        updated_rac = [existing_rac rac_col];
    else
        updated_rac = rac_col;
    end
    writematrix(updated_rac, rac_file);
    
    % Append to rbc.csv
    if exist(rbc_file, 'file')
        existing_rbc = readmatrix(rbc_file);
        updated_rbc = [existing_rbc rbc_col];
    else
        updated_rbc = rbc_col;
    end
    writematrix(updated_rbc, rbc_file);
