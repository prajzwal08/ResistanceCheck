function [resist_out] = resistancesnew(resist_in)
    ...
    k_mu_soil  = 1.327 * 10^(-5) * (po/p) * (Ta/To); % KInematic viscosity 
    ...
    Re_soil     = h_soil*u_soil/k_mu_soil;
    St_nu       = 2.46 * ((Re_soil) ^ (1/4)) - log(7.4); % Stanton Number (KB-1)
    ...
    rbs = St_nu / (kappa * ustar); % Boundary layer resistance of soil for bare soil.
    ...
    resist_out.rbs = rbs;   % Boundary layer of bare soil.

    rac  = rai + rar + rawc;  % YT(aerodynamic , within canopy)
    rash = rai + rar + raws ; % Aerodynamic (soil, within canopy (shaded))
    rasu = rai + rar + rbs;   % Boundary soil (sunlight)

    resist_out.ustar = ustar;
    resist_out.uh = uh;
    resist_out.rac  = rac;         
    resist_out.rahs = rash;        
